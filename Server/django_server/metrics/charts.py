...
class RELineChart():
    def __init__(self, **kwargs):
        ...
        self.chart.title = 'Room environment temperature'
    ...
    def get_data(self,type):
        ...
        re_objects = Room_environment.objects.filter(ts__gte=ts_threshold)
        for re in re_objects:
            print(re)
            if type == "data":
                list.append(re.temperature)
            elif type == "x_labels":
                clock_time = datetime.datetime.fromtimestamp(re.ts).time() #ts to readable clock time
